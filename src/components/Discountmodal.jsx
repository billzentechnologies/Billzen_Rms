          ...
          <div className="grid grid-cols-3 gap-2">
            {discounts.filter(d => d.isActive && d.discountPercentage > 0).map((discount) => (
              <button
                ...
                className={`p-3 rounded-lg font-bold text-sm border-2 transition-all ${(selectedDiscount?.discountId === discount.discountId) ||
                    (selectedDiscount?.discountPercentage === discount.discountPercentage)
