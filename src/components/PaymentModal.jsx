...
const PaymentModal = ({
  ...
  const calculateBillTotals = () => {
    ...
    selectedItems.forEach((item, itemIndex) => {
      ...
      const itemIndividualDiscount = (item.discount || 0) * item.quantity;
  ...
  const ensureOrderExists = async () => {
    ...
    const shouldSave = newItems.length > 0 || voidedItems.length > 0 || actualOrderId === 0 || selectedDiscount !== null;
    ...
    try {
      ...
      // Bill-level discount amount for SaveOrder (same as Payment modal "Bill Discount")
  ...
  // Modified to auto-bill when payment mode is selected
    ...
    try {
      ...
      const checkPrintPayload = {
        CheckPrintRequest: {
          ...
          staffId: user?.id || 12,
          printType: 0,
          discountPercentage: selectedDiscount?.discountPercentage || 0
        }
      };
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    {totals.billLevelDiscount > 0 && (
                      <div className="flex justify-between items-center text-green-600">
                        <span className="flex items-center gap-1">
                          <FaPercentage className="text-[10px]" />
                          Bill Discount ({selectedDiscount?.discountPercentage}%)
                        </span>
                        <span className="font-semibold">
